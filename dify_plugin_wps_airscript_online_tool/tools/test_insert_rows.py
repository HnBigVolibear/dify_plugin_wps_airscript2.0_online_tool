...
class test_insert_rows(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = client.insert_rows(row_index, count, sheet_name)
            if result and result[0]['success']:
                pass
            else:
                raise Exception(f"插入行失败！WPS官方返回错误信息：{result}")
        except Exception as e:
            raise Exception(f"插入行失败！WPS官方返回错误信息：{e}")

        yield self.create_text_message(f"插入行成功！在第{row_index}行位置插入了{count}行\n")
