...
class test_unmerge_cells(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = client.unmerge_cells(address, sheet_name)
            if result and result[0]['success']:
                pass
            else:
                raise Exception(f"取消合并单元格失败！WPS官方返回错误信息：{result}")
        except Exception as e:
            raise Exception(f"取消合并单元格失败！WPS官方返回错误信息：{e}")

        yield self.create_text_message(f"取消合并单元格成功！区域：{address}\n")
