...
class test_get_cell_formula(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = client.get_cell_formula(address, sheet_name)
            # print(result) # [{'formula': '=A1&B1', 'success': True}]
            if result and result[0]['success']:
                formula = result[0]['formula']
                yield self.create_text_message(str(formula)+"\n")
                yield self.create_json_message({"formula": formula})
            else:
                yield self.create_text_message("该单元格没有公式！\n")
        except Exception as e:
            raise Exception(f"获取单元格公式失败！WPS官方返回错误信息：{e}")
