...
class test_set_background_color(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = client.set_background_color(address, color, sheet_name)
            if result and result[0]['success']:
                pass
            else:
                raise Exception(f"设置背景色失败！WPS官方返回错误信息：{result}")
        except Exception as e:
            raise Exception(f"设置背景色失败！WPS官方返回错误信息：{e}")

        yield self.create_text_message("设置背景色成功！\n")
