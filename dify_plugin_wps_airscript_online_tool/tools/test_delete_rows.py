...
class test_delete_rows(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = client.delete_rows(row_index, count, sheet_name)
            if result and result[0]['success']:
                pass
            else:
                raise Exception(f"删除行失败！WPS官方返回错误信息：{result}")
        except Exception as e:
            raise Exception(f"删除行失败！WPS官方返回错误信息：{e}")

        yield self.create_text_message(f"删除行成功！从第{row_index}行开始删除了{count}行\n")
