...
class test_insert_columns(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = client.insert_columns(column_index, count, sheet_name)
            if result and result[0]['success']:
                pass
            else:
                raise Exception(f"插入列失败！WPS官方返回错误信息：{result}")
        except Exception as e:
            raise Exception(f"插入列失败！WPS官方返回错误信息：{e}")

        yield self.create_text_message(f"插入列成功！在第{column_index}列位置插入了{count}列\n")
